...
| T15 | impl1 | `Cargo.toml` only: replace the `kdl` line's comment ("Already compiled in through zellij-utils, which does not re-export it.") with "In the dependency tree via zellij-utils, which neither re-exports it nor links it — first real use costs ~153 KiB of wasm." (rev 11 retracted the premise the old comment states). | — | [ ] |
...
| T11 | impl3 | Run the PLAN's Verification on the merged + tidied state, E2E included — preflight first, then run it unattended when it fits; artifacts to `/tmp/layout-generator/`. On failure or preflight no-go, report to the planner and wait for the routed fix — never self-fix, never free up resources; loop until clean. | T10, T15 | [ ] |
