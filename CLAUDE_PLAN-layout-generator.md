...
tab { ... }                                       // no name: "<source tab>-<ordinal>", 1-based, in emission order
each for="k" in="from..from+m" { tab "{tab}-crit{k}" { ... } }   // a..b / a..=b, Rust semantics; endpoint = term [('+'|'-') term], term = int | name; one operator, no chaining
pane "claude -n impl{i}"                          // {name} substituted for declared names only; {tab} arrives shell-quoted here; other braces verbatim
```
- Structure: `each` nests anywhere; `tab` only at top level or inside top-level `each`; `pane` only inside a `tab`. Unknown node, property, or variable name is a parse error. Variables: positionals, flag values, flag presences, `each` loop variables, and the built-in `tab` (the source tab's name, the only string); all others are integers or presences.
