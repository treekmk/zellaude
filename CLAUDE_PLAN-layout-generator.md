...
Replace enumerated custom states with generator files: one KDL file per generator under `~/.config/zellij/plugins/zellaude/generators/`, invoked from the existing `Ctrl+t` `Shift+n` prompt with a CLI-style line (`impl 4 --crit-per-impl 2`). A small declarative language (`tab`/`pane`/`each`, `if`/`unless`, integer ranges) expands the file into one or more tabs, named `<source tab>-1`, `-2`, … unless a name template using `{tab}` and the loop variables says otherwise; the plugin lays each tab out from the live tab size under minimum-pane floors and refuses when it does not fit. Generator files and the custom-state portion of `zellaude.json` are re-read when the prompt opens and again on every Enter, so edits need no new session and a refusal is fixed by editing the named file and pressing Enter again. No new dependency in the tree: the `kdl` crate already reaches the plugin through zellij-utils and needs only a manifest line; this feature is its first real use in the plugin, so the parser now links (+153 KiB on a 1.7 MB wasm).
...
- Phase 3 (T5–T6, impl2): `src/main.rs` + `src/state.rs` — reload on prompt open through one `run_command` returning `zellaude.json` plus the generator files; a result arm that touches only the custom-state portion; submit always records the input and reloads, then launches exact id first, generator second, with geometry from `TabInfo`; `mod layout_generators` wired.
- Phase 4 (T7 impl1, T8 impl2): README; E2E harness under `/tmp/layout-generator/e2e/`.
- Phase 3b (T13 impl2, T14 impl1): reload at every submit through the non-deferring `launch_custom_state`, its inline test and E2E case 4; then the README's reload sentences follow the behaviour.
- Finalize (T9–T12, impl3).
- Cross-phase: T6 waits on T3 (generator API) and T4 (emitter); T7 on T3; T8 on T6; T13 on T6 and T8; T14 on T13; T9 on both.
...
- `src/main.rs` — `RELOAD_CUSTOM_STATES_SCRIPT` beside `SAVE_CONFIG_SCRIPT` (`:27`); `start_custom_layout_prompt` (`:637`) issues the reload; `open_custom_layout` (`:719`) records the submit and reloads; `launch_custom_state` resolves when the result lands; new `RunCommandResult` arm `reload_custom_states` beside `load_config` (`:311`); `mod layout_generators`.
...
- `README.md` — "Custom states" (`:55-111`): "Reload the plugin" (`:79`) replaced by hot reload; the Zellij-fit sentence (`:83`) stays for fixed states, which the floors never touch, and points to the Generators section; new "Generators" subsection; Features bullet (`:16`); after T13, the reload sentences say "when the prompt opens and again when you press Enter" (T14).
...
- Every generator refusal is prefixed with the file's basename and contains `does not fit` when a floor is the cause. The generator validates each resolved tab name itself (non-empty, no leading or trailing whitespace, at most `MAX_ID_CHARACTERS`, no control characters) before it becomes a `CustomLayout` id, so a bad `{tab}` refuses in the generator's voice, never in `CustomLayout::validate`'s. That pre-check covers names only: the command byte caps and the NUL check stay `CustomLayout::validate`'s, which `invoke` runs on every emitted `CustomLayout` before returning and prefixes with the file basename like every other refusal (the submit path never knows the file). A single `each` range is bounded before iteration: more than `MAX_PANES` steps refuses (`range "1..=n" runs for N steps; the maximum is 64`), since a tab cannot hold more and more tabs than that from one prompt is not a use case; a prompt-line typo like `impl 1000000000` therefore never allocates. Nested `each` levels inside a trusted file multiply past that bound: panes are caught by the per-tab cap, and tabs by `MAX_GENERATED_TABS` per invocation, checked at the moment each tab is created and refusing as soon as the count passes the cap (`<file>: this invocation opens more than 64 tabs`; a post-expansion count would let a three-deep file build millions of command strings before speaking), so no invocation hands Zellij an unbounded document. The one unprefixed refusal is the no-match case: when no `command` equals the first token, `invoke` returns `Unknown custom state or generator "…"` (no file matched), and `resolve_custom_state` passes every `invoke` error through verbatim, retiring today's `Unknown custom state` string.

```rust
// src/layout_generators.rs — one generator = one .kdl file, re-parsed when the prompt opens and again on every Enter
...
pub const MAX_GENERATED_TABS: usize = 64;    // tabs one invocation may open; same number as MAX_PANES, different quantity
...
pub struct Prompt { ..., pub pending_submit: Option<String> }   // new field, set in Prompt::new; every submitted input, resolved when the reload result lands
```
...
// src/main.rs
const RELOAD_CUSTOM_STATES_SCRIPT: &str = ...;   // sh + jq: prints the CustomStateSources envelope; zellaude.json missing → "{}"; *.kdl only, LC_ALL=C order; zellaude.json read by `--rawfile`, one object per file written to a pipe, wrapped once by `jq -s` — nothing large goes through argv (a single argv string is capped at 128 KiB)
fn reload_custom_states(&mut self) { ... }                                   // run_command with context type "reload_custom_states"; no in-flight flag — ordering comes from record-then-reload
...
fn launch_custom_state(&mut self, input: &str) -> bool { ... }   // the non-deferring path: source tab, resolve_custom_state, tabs_to_kdl, new_tabs_with_layout, errors to prompt.error; called only by resolve_pending_submit, never reloads
...
- Unit coverage, `src/main.rs` inline: `RELOAD_CUSTOM_STATES_SCRIPT` run through real `sh` against a temp `HOME` (pattern of the `save_config` tests, `src/main.rs:2004-2120`): missing file → `settings_json == "{}"`, files in byte order, non-`.kdl` ignored, and a 200 KiB generator file and a 200 KiB `zellaude.json` each round-trip intact (an argv-borne document fails at 128 KiB). `apply_custom_state_sources` on a `State`: a document without `custom_states` clears previously loaded states, a document that fails to parse keeps them and fills the error slot, a bad generator file keeps the last good generators and fills `layout_generator_config_error`, and with `custom_layouts_from_plugin_configuration` set a document without `custom_states` leaves the plugin-block states intact.
- **E2E** — a real Zellij session drives the built plugin through the prompt.
  ...
  - Drive: a python `pty.fork` client (200 columns × 50 rows via `TIOCSWINSZ`) runs `zellij --session zellaude-e2e-lg --new-session-with-layout e2e.kdl` under the staged `HOME` (`--session` with `--layout` adds tabs to an existing session and fails with `Session not found`), waits for the bar, and before every case sends `Esc` (a refusal leaves the prompt open, and a second `Ctrl+t` `N` would append to it) and returns focus to the `e2e` tab (`zellij -s zellaude-e2e-lg action go-to-tab-name e2e`, confirmed by the `FOCUSED` column of `list-panes` before typing, since case 1 leaves focus on the first generated tab), then sends `Ctrl+t`, `N`, types the line, `Enter`, and captures the screen. Assertions read `zellij -s zellaude-e2e-lg action list-panes --all --tab --geometry`.
  - Cases and pass conditions: (1) `grid 4 --tabs 2` → tabs `e2e-1` and `e2e-2` exist (the default names), each with exactly four terminal panes whose titles end in `-lc printf g1-p1; sleep 600` … (Zellij titles a command pane with its whole argv), every pane `COLS` = 200 and `ROWS` ≥ 11 (width floor 100 forces one column, four rows). (2) `grid 9` → tab count unchanged after 3 s and the captured screen contains `does not fit`. (3) write `generators/hot.kdl` (`command "hot"`, `arg "n"`, `tab "{tab}-hot"` holding *n* panes) after the session is up, then `hot 2` → tab `e2e-hot` with two panes, proving the reload without a restart and the `{tab}` template. (4) add `min_pane_height 40` to `hot.kdl` (a file that parses but cannot fit; a parse-broken file would keep the last good set and never refuse), type `hot 3` → the refusal names `hot.kdl`, contains `does not fit`, and the prompt stays open; remove the line and press `Enter` again with no other key → a new tab id named `e2e-hot` with exactly three panes (asserted by tab id, since case 3's tab remains), proving Enter resolves against the files as they are at that moment. Exit 0 only when all four hold.
...
- **KDL, not YAML or JSON:** the `kdl` crate is already in the dependency tree (zellij-utils), Zellij users already write it, and the plugin already emits it. T11 measured that it had been dead-code-eliminated from the shipped wasm until now: first real use links the parser for +156,846 B (+8.8%), not the near-zero the draft assumed. The dependency stays — a second format or a hand-written parser would cost more than 153 KiB is worth. Rust's YAML crates are unsettled (`serde_yaml` deprecated, `serde_yml` unmaintained); JSON makes the declarations noisy. zellij-utils does not re-export `kdl` (checked), hence the manifest line.
...
- **Reload trigger:** on prompt open (refreshes the hint) and on every submit — `open_custom_layout` (the prompt's Enter) only records `pending_submit` and issues the reload; the result arm's `resolve_pending_submit` then calls `launch_custom_state`, the non-deferring path that never reloads and is called from nowhere else, so the cycle is exactly one deep and Enter resolves against the files as they are at that moment. A refusal leaves the prompt open with the input intact, and the next Enter after an edit picks the edit up without cancelling; reloading only at open would have resolved that Enter against the pre-edit read (found by plan-crit1 once T8 showed the prompt survives a refusal). Cost: one shell round trip per open and per submit — a `cat` plus one small `jq` per generator file, wrapped once. No polling: the wasm plugin has no file watch and a timer `stat` is waste. A prompt cancelled during a reload drops the pending submit. There is no in-flight flag (T13 removed its only reader): the guarantee is record-then-reload plus the one-deep cycle. The one accepted reorder is an open-time read landing after an Enter pressed within its millisecond span, which resolves against a read begun at prompt open — harmless, since nothing can be edited between opening the prompt and that Enter.
