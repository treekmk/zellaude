...
```kdl
// ~/.config/zellij/plugins/zellaude/generators/<name>.kdl — declaration nodes, then body nodes
...
min_pane_height 12
tab "impl" unless="single_tab only_crit" { ... }  // if="a b": all present; unless="a b": none present
each for="k" in="from..from+m" { tab "crit{k}" { ... } }   // a..b / a..=b, Rust semantics; endpoint = term [('+'|'-') term], term = int | name; one operator, no chaining
pane "claude -n impl{i}"                          // {name} substituted for declared names only; other braces verbatim
```
- Structure: `each` nests anywhere; `tab` only at top level or inside top-level `each`; `pane` only inside a `tab`. Unknown node, property, or variable name is a parse error. Variables: positionals, flag values, flag presences, `each` loop variables; all integers except presences.
- Prompt line: tokens split on whitespace; first token is the command; positionals fill in order and may sit among flags; flags in any order. Unknown flag, missing or non-integer value, or leftover token refuses.
...
- Every generator refusal is prefixed with the file's basename and contains `does not fit` when a floor is the cause.
...
pub fn invoke(generators: &[LayoutGenerator], input: &str, geometry: TabGeometry, global_floors: FloorOverrides) -> Result<Vec<CustomLayout>, String> { ... }   // impl1 produces; impl2 calls from the submit path; one CustomLayout per emitted tab, id = tab name
...
fn resolve_custom_state(&self, input: &str, geometry: TabGeometry) -> Result<Vec<CustomLayout>, String> { ... }   // exact custom_states id → clone; else layout_generators::invoke
fn prompt_tab_geometry(&self, tab_position: usize, chrome: &TabChrome) -> Option<TabGeometry> { ... }   // TabInfo.display_area_* of that position, rows minus chrome.bar_rows
```
...
- Unit coverage, `tests/layout_generators.rs`: parse errors (unknown node/property/variable, duplicate variable, `tab` inside `tab`, `pane` outside `tab`); prompt-line parsing (flags in any order, optional value present/absent, defaults, unknown flag, missing value, non-integer, leftover token); ranges (`..`, `..=`, `+`/`-` with an integer and with a variable, overflow and negative bound refuse, empty); `if`/`unless` all/none; nested `each` order (i-major); `plan_rows` table (W=284: c_max=5, n=7 → [3,4], n=12 → [4,4,4]; width refusal at c_max=0; height refusal); floors chain file → global → constants; duplicate `command` across files names both; `{i}` substituted while `${HOME}` and `{a,b}` stay; the madev file from the README expands `impl 4 --crit-per-impl 2` to tabs `impl`, `crit1`, `crit2`, `impl 4 --single-tab` to one 12-pane tab, and `impl 4 --crit-per-impl 2 --only-crit 2` to tabs `crit2`, `crit3` only; every generated tab round-trips through `Layout::from_kdl` with the expected positioned geometry.
...
- Unit coverage, `src/main.rs` inline: `RELOAD_CUSTOM_STATES_SCRIPT` run through real `sh` against a temp `HOME` (pattern of the `save_config` tests, `src/main.rs:2004-2120`): missing file → `settings_json == "{}"`, files in byte order, non-`.kdl` ignored.
- **E2E** — a real Zellij session drives the built plugin through the prompt.
  - Setup: build the wasm; stage `HOME=/tmp/layout-generator/e2e/home` (never the real one) with `.config/zellij/plugins/zellaude.json` = `{"min_pane_width": 100, "min_pane_height": 5}`, `generators/grid.kdl` (`command "grid"`, `arg "n"`, `flag "tabs" value="t" default=1`, `each for="k" in="1..=t" { tab "g{k}" { each for="i" in="1..=n" { pane "printf g{k}-p{i}; sleep 600" } } }`), `.cache/zellij/permissions.kdl` granting the built wasm's absolute path the seven `REQUIRED_PERMISSIONS`, and `e2e.kdl` = the repo's `layout.kdl` pointing at that wasm.
  - Drive: a python `pty.fork` client (200 columns × 50 rows via `TIOCSWINSZ`) runs `zellij --session zellaude-e2e-lg --layout e2e.kdl` under the staged `HOME`, waits for the bar, sends `Ctrl+t`, `N`, types the line, `Enter`, and captures the screen. Assertions read `zellij -s zellaude-e2e-lg action list-panes --all --tab --geometry`.
  - Cases and pass conditions: (1) `grid 4 --tabs 2` → tabs `g1` and `g2` exist, each with exactly four terminal panes titled `printf g1-p1; sleep 600` …, every pane `COLS` = 200 and `ROWS` ≥ 11 (width floor 100 forces one column, four rows). (2) `grid 9` → tab count unchanged after 3 s and the captured screen contains `does not fit`. (3) write `generators/hot.kdl` (`command "hot"`, `arg "n"`, one tab of *n* panes) after the session is up, then `hot 2` → tab `hot` with two panes, proving the reload without a restart. Exit 0 only when all three hold.
...
- **Integer and presence variables only:** a substituted value is always a digit string, so it cannot change how `sh -lc` parses a command. The only code in a file is its pane commands — the same trust `custom_states` already carries (README "trusted local shell code").
