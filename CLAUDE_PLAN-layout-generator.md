...
- **Reload trigger:** on prompt open (refreshes the hint) and on every submit — `open_custom_layout` (the prompt's Enter) only records `pending_submit` and issues the reload; the result arm's `resolve_pending_submit` then calls `launch_custom_state`, the non-deferring path that never reloads and is called from nowhere else, so the cycle is exactly one deep and Enter resolves against the files as they are at that moment. A refusal leaves the prompt open with the input intact, and the next Enter after an edit picks the edit up without cancelling; reloading only at open would have resolved that Enter against the pre-edit read (found by plan-crit1 once T8 showed the prompt survives a refusal). Cost: one `cat` per submit. No polling: the wasm plugin has no file watch and a timer `stat` is waste. A prompt cancelled during a reload drops the pending submit. One boolean flag: the window is one `cat`, so overlapping reloads apply in arrival order and are accepted.
